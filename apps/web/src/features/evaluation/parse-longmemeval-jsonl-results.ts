import type {
  LongMemEvalJsonlResultItem,
  LongMemEvalJsonlResultPage,
  LongMemEvalJsonlResultSummary
} from "./request-longmemeval-evaluation.js";

export function parseLongMemEvalJsonlText(input: {
  text: string;
  fileName: string;
  page: number;
  pageSize: number;
  ...
  const summary = createEmptySummary(input.fileName);
  ...
  for (const [index, line] of lines.entries()) {
    ...
    try {
      ...
      const isTail = index === lines.length - 1 && !input.text.endsWith("\n") && !input.text.endsWith("\r");
      if (isTail) {
        summary.ignoredIncompleteTail = true;
        continue;
      }
      throw error;
    }
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      throw new Error(`JSONL 第 ${index + 1} 行必须是 JSON object`);
    }
    const item = normalizeLongMemEvalJsonlResult(parsed, index + 1);
    const identityKey = readResultIdentityKey(parsed, item);
    if (identityKey) latestItems.set(identityKey, item);
    else legacyItems.push(item);
  }
  const allItems = [...legacyItems, ...latestItems.values()].sort(compareLongMemEvalItems);
  for (const item of allItems) addJsonlResultToSummary(summary, item);
...
function normalizeLongMemEvalJsonlResult(record: Record<string, unknown>, lineNumber: number): LongMemEvalJsonlResultItem {
  ...
  const answerFallbackUsed = readBoolean(record.answerFallbackUsed) ?? Boolean(answerFallbackReason);
...
function createEmptySummary(fileName: string): LongMemEvalJsonlResultSummary {
  return {
    filePath: fileName,
    totalItems: 0,
    judgedItems: 0,
...
function addJsonlResultToSummary(summary: LongMemEvalJsonlResultSummary, item: LongMemEvalJsonlResultItem) {
  ...
  const modelGroup = summary.modelGroups[modelRunId] ??= { modelRunId, totalItems: 0, judgedItems: 0, correctItems: 0 };
  ...
  if (summary.judgedItems > 0) summary.accuracy = summary.correctItems / summary.judgedItems;
  if (summary.totalItems > 0) summary.exactMatchAccuracy = summary.exactMatchItems / summary.totalItems;
  if (typeSummary.judged > 0) typeSummary.accuracy = typeSummary.correct / typeSummary.judged;
  if (modelGroup.judgedItems > 0) modelGroup.accuracy = modelGroup.correctItems / modelGroup.judgedItems;
}
...
function compareLongMemEvalItems(left: LongMemEvalJsonlResultItem, right: LongMemEvalJsonlResultItem) {
  ...
  return (left.sampleIndex ?? Number.MAX_SAFE_INTEGER) - (right.sampleIndex ?? Number.MAX_SAFE_INTEGER) || left.lineNumber - right.lineNumber;
}
...
function readDroppedReasons(value: unknown) {
  ...
  for (const item of toObjectArray(value)) {
    increment(result, readString(item.reason) ?? "unknown");
  }
  return result;
}
